...
- `Sound.resume(id?)` — resume a paused voice by id, or resume all currently-paused voices and return the last resumed id (or `-1` if none were paused); throws `AudioDisposedError` after `dispose()`.
